...
class BallDetector:
    ...
    def __init__(self, save_state, out_channels=2):
        ...
        saved_state_dict = torch.load(save_state, map_location='cpu')
    ...
    def detect_ball(self, frame):
        ...
        if self.last_frame is not None:
            ...
            #self.xy_coordinates = np.array(self.xy_coordinates, dtype=np.float)
            #_x, _y = self.xy_coordinates[:, 0], self.xy_coordinates[:, 1]
            #_xnew = _x[~np.isnan(_x)]
            #_ynew = _y[~np.isnan(_y)]
            #self.xy_coordinates = np.stack((_xnew, _ynew)).T
